...
from AnalysisFunction import Gps_Analyze
from DataRead import Read_file
from Post_processing import GPS_Post_processing
from config import FileConfig, GpsConfig, OutputConfig
...
file_path = gen_filenames[file_conf.gen_filenames](file_conf.start_time, file_conf.end_time, file_conf.base_dir,
                                                   file_conf.filename_patterns['GPS'], file_conf.date_pattern,
                                                   file_conf.time_pattern)
file_list_df = spark.createDataFrame(file_path, ["timestamp", "file_path"])

# 将自定义分析函数注册为 UDF
analyze_file_udf = F.udf(lambda File_path: Gps_Analyze.Gps_analyze(File_path, gps_conf),
...
for task in gps_conf.tasks_channels:
...
df_with_selected_columns.coalesce(8).write.csv("result_folder_gps", header=True, mode='overwrite')
...
folder_path = os.path.join(current_dir, 'result_folder_gps')
os.makedirs(folder_path, exist_ok=True)
# 将结果分别保存到csv文件中用于后续绘图分析
GPS_Post_processing.GPS_rlt_read(folder_path, gps_conf)
# 关闭当前会话
spark.stop()
...
figure_path = outputconfig['GPS']['preprocess']['raw_data_figure_path']
for i in range(len(file_path)):
    data, file_exists = Read_file.Read_csv1(file_path[i][1])
    channels = outputconfig['GPS']['preprocess']['channels']
    try:
    ...
    if file_exists:
        for j in range(len(figure_path)):
            ...
            channel=outputconfig['GPS']['preprocess']['channels'][j]
            ...
            plt.title(file_path[i][0].strftime("%y.%m.%d-%H")+f"时{channel}号GPS传感器时程图")
