...
from Post_processing import WDR_Post_processing
...
# 将自定义分析函数注册为 UDF
analyze_file_udf = F.udf(lambda File_path: Wdr_Analyze.Wdr_analyze(File_path, wdr_conf),
...
for task in wdr_conf.tasks_channels:
...
df_with_selected_columns.coalesce(10).write.csv("result_folder_wdr", header=True, mode='overwrite')

# 分析结果
# 获取当前脚本所在路径
current_dir = os.path.dirname(os.path.abspath(__file__))
# 构建相对路径到'../data/results/'文件夹
folder_path = os.path.join(current_dir, 'result_folder_wdr')
os.makedirs(folder_path, exist_ok=True)
# 将结果分别保存到csv文件中用于后续绘图分析
WDR_Post_processing.WDR_rlt_read(folder_path, wdr_conf)
##################绘制原始数据的时程图#####################################
# 设置全局字体为宋体，字号为12
