...
from AnalysisFunction import Set_Analyze
from DataRead import Read_file
from Post_processing import SET_Post_processing
from config import FileConfig, SetConfig, OutputConfig
...
# 将自定义分析函数注册为 UDF
analyze_file_udf = F.udf(lambda File_path: Set_Analyze.Set_analyze(File_path, set_conf),
...
for task in set_conf.tasks_channels:
...
df_with_selected_columns.coalesce(4).write.csv("result_folder_set", header=True, mode='overwrite')
...
folder_path = os.path.join(current_dir, 'result_folder_set')
os.makedirs(folder_path, exist_ok=True)
# 将结果分别保存到csv文件中用于后续绘图分析
SET_Post_processing.SET_rlt_read(folder_path, set_conf)
# 关闭当前会话
spark.stop()
