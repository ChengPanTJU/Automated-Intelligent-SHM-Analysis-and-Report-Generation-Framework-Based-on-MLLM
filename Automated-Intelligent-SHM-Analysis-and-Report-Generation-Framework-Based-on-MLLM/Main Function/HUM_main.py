...
from AnalysisFunction import Hum_Analyze
from DataRead import Read_file
from Post_processing import HUM_Post_processing
from config import FileConfig, HumConfig, OutputConfig
...
# 将自定义分析函数注册为 UDF
analyze_file_udf = F.udf(lambda File_path: Hum_Analyze.Hum_analyze(File_path, hum_conf),
...
for task in hum_conf.tasks_channels:
...
df_with_selected_columns.coalesce(20).write.csv("result_folder_hum", header=True, mode='overwrite')
...
folder_path = os.path.join(current_dir, 'result_folder_hum')
os.makedirs(folder_path, exist_ok=True)
# 将结果分别保存到csv文件中用于后续绘图分析
HUM_Post_processing.HUM_rlt_read(folder_path, hum_conf)
