...
from Post_processing import TMP_Post_processing
...
# 将自定义分析函数注册为 UDF
analyze_file_udf = F.udf(lambda File_path: Tmp_Analyze.Tmp_analyze(File_path, tmp_conf),
...
for task in tmp_conf.tasks_channels:
...
df_with_selected_columns.coalesce(20).write.csv("result_folder_tmp", header=True, mode='overwrite')
...
folder_path = os.path.join(current_dir, 'result_folder_tmp')
os.makedirs(folder_path, exist_ok=True)
# 将结果分别保存到csv文件中用于后续绘图分析
TMP_Post_processing.TMP_rlt_read(folder_path, tmp_conf)
