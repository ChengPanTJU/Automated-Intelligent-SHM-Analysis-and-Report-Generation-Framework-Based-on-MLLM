...
from AnalysisFunction import Ian_Analyze
from DataRead import Read_file
from Post_processing import IAN_Post_processing
from config import FileConfig, IanConfig, OutputConfig
...
# 将自定义分析函数注册为 UDF
analyze_file_udf = F.udf(lambda File_path: Ian_Analyze.Ian_analyze(File_path, ian_conf),
...
for task in ian_conf.tasks_channels:
...
df_with_selected_columns.coalesce(10).write.csv("result_folder_ian", header=True, mode='overwrite')
...
folder_path = os.path.join(current_dir, 'result_folder_ian')
os.makedirs(folder_path, exist_ok=True)
# 将结果分别保存到csv文件中用于后续绘图分析
IAN_Post_processing.IAN_rlt_read(folder_path, ian_conf)
