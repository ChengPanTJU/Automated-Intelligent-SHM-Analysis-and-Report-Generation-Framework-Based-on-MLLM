...
from AnalysisFunction import Vic_Analyze
from DataRead import Read_file
from Post_processing import  VIC_Post_processing
from config import FileConfig, VicConfig, OutputConfig
...
file_path = gen_filenames[file_conf.gen_filenames](file_conf.start_time, file_conf.end_time, file_conf.base_dir,
                                                   file_conf.filename_patterns['cable_vib'], file_conf.date_pattern,
                                                   file_conf.time_pattern)
file_list_df = spark.createDataFrame(file_path, ["timestamp", "file_path"])

# 将自定义分析函数注册为 UDF
analyze_file_udf = F.udf(lambda File_path: Vic_Analyze.Vic_analyze(File_path, vic_conf),
...
for task in vic_conf.tasks_channels:
...
df_with_selected_columns.coalesce(20).write.csv("result_folder_vic", header=True, mode='overwrite')

# 分析结果
# 获取当前脚本所在路径
current_dir = os.path.dirname(os.path.abspath(__file__))
# 构建相对路径到'../data/results/'文件夹
folder_path = os.path.join(current_dir, 'result_folder_vic')
os.makedirs(folder_path, exist_ok=True)
# 将结果分别保存到csv文件中用于后续绘图分析
VIC_Post_processing.VIC_rlt_read(folder_path, vic_conf)
...
figure_path = outputconfig['cable_vib']['preprocess']['raw_data_figure_path']
for i in range(len(file_path)):
    data, file_exists = Read_file.Read_csv1(file_path[i][1])
    channels = outputconfig['cable_vib']['preprocess']['channels']
    try:
    ...
    if file_exists:
        for j in range(len(figure_path)):
            ...
            channel=outputconfig['cable_vib']['preprocess']['channels'][j]
            ...
            plt.title(file_path[i][0].strftime("%y.%m.%d-%H")+f"时拉索{channel}号加速度计时程图")
