...
from config import OutputConfig, WsdConfig, FileConfig
import AnalysisFunction.analysis_methods as AM
OutConfig = OutputConfig()
outputconfig = OutConfig.tasks

def WSD_rlt_read(folder_path, wsd_conf):
    # 任务与分析函数的映射
    rlt_functions = {
        'preprocess': Prep_rlt,
        'rms': rms_rlt,  # rms函数
...
def Prep_rlt(rlt, rlt_combined):
    ...
    preprocess_path = outputconfig['wind_speed']['preprocess']['save_path']
    ...
    # 生成列表
    channels_list = outputconfig['wind_speed']['preprocess']['channels']
    ytick_labels = [f"WSD{str(i).zfill(2)}" for i in channels_list]
    ...
    plt.title(title_name[0] + "至" + title_name[len(x_t) - 1] + "风速仪情况")
    # 调整图形布局
    plt.tight_layout()  # 自动调整布局以防止标签重叠
    figure_path = outputconfig['wind_speed']['preprocess']['figure_path']
...
def rms_rlt(rlt, rlt_combined):
    ...
    rms_path = outputconfig['wind_speed']['rms']['save_path']
    rms = pad_lists(rlt['rms'].tolist(), len(outputconfig['wind_speed']['rms']['channels']))
    ...
    result = pd.DataFrame({
        '通道号': outputconfig['wind_speed']['rms']['channels'],  # 设置第一列为通道号
        '风速均方根最大值(m/s)': save_data.max().values,  # 最大值
        '最大值时刻(月.日-时)': save_data.idxmax().values,  # 最大值所在的行号
        '风速均方根最小值(m/s)': save_data.min().values,  # 最小值
    ...
    sum_data_path = outputconfig['wind_speed']['rms']['sum_table_path']
    result.to_csv(sum_data_path, index=False)
    ##############################绘图#################################
    pic_num = len(outputconfig['wind_speed']['rms']['figure_path'])
    x_t = rlt['timestamp'].dt.tz_localize(None)  # 时间轴
    rms = save_data.values.tolist()
    rms = pad_lists(rms, len(outputconfig['wind_speed']['rms']['channels']))
    for pic in range(pic_num):
        single_rms = [rms[i][pic] for i in range(len(rms))]
        figure_path = outputconfig['wind_speed']['rms']['figure_path'][pic]
        channel_num = outputconfig['wind_speed']['rms']['channels'][pic]
        fig = plt.figure(figsize=(fig_width, fig_height))
        plt.plot(x_t, single_rms, label='rms',linewidth=0.2)
        # 设置 x 轴标签格式为 "日-月 时"
        title_name = rlt['timestamp'].dt.strftime("%y.%m.%d")
        plt.title(f'{title_name[0]}至{title_name[len(x_t) - 1]}日{channel_num}号风速仪均方根')
        plt.xlabel('时间 (月-日）')
        plt.ylabel('风速均方根 (m/s)')
...
def average_rlt(rlt, rlt_combined):
    ...
    mean_path = outputconfig['wind_speed']['mean']['save_path']
    rms = pad_lists(rlt['mean'].tolist(), len(outputconfig['wind_speed']['mean']['channels']))
    save_data = pd.DataFrame(rms, index=None)
    for i in range(save_data.shape[1]):
        original_data = save_data.iloc[:,i].copy()
        save_data.iloc[:,i],_=AM.rmoutliers_gesd(original_data)
    ...
    time_data = rlt['timestamp'].dt.strftime("%m.%d-%H")
    result = pd.DataFrame({
        '通道号': outputconfig['wind_speed']['mean']['channels'],  # 设置第一列为通道号
        '风速均值最大值(m/s)': save_data.max().values,  # 最大值
        '最大值时刻(月.日-时)': save_data.idxmax().values,  # 最大值所在的行号
        '风速均值最小值(m/s)': save_data.min().values,  # 最小值
        '最小值时刻(月.日-时)': save_data.idxmin().values  # 最小值所在的行号
    })
    # 用 time_data 的索引值替换 result 中的索引
    result['最大值时刻(月.日-时)'] = result['最大值时刻(月.日-时)'].apply(
    ...
    # 填充 NaN
    result = result.fillna('通道数据缺失')
    sum_data_path = outputconfig['wind_speed']['mean']['sum_table_path']
    result.to_csv(sum_data_path, index=False)
    ##############################绘图#################################
    pic_num = len(outputconfig['wind_speed']['mean']['figure_path'])
    x_t = rlt['timestamp'].dt.tz_localize(None)  # 时间轴
    rms = save_data.values.tolist()
    rms=pad_lists(rms,len(outputconfig['wind_speed']['mean']['channels']))
    for pic in range(pic_num):
        single_rms = [rms[i][pic] for i in range(len(rms))]
        figure_path = outputconfig['wind_speed']['mean']['figure_path'][pic]
        channel_num = outputconfig['wind_speed']['mean']['channels'][pic]
        fig = plt.figure(figsize=(fig_width, fig_height))
        plt.plot(x_t, single_rms, label='mean',linewidth=0.2)
        # 设置 x 轴标签格式为 "日-月 时"
        title_name = rlt['timestamp'].dt.strftime("%y.%m.%d")
        plt.title(f'{title_name[0]}至{title_name[len(x_t) - 1]}日{channel_num}号风速仪均值')
        plt.xlabel('时间 (月-日）')
        plt.ylabel('风速均值(m/s)')
        plt.tick_params(axis='y')
        plt.tick_params(axis='x')
...
folder_path = r'D:\论文\04自动生成报告\监测数据自动化分析\result_folder_wsd'
WSD_rlt_read(folder_path, vicconfig)
...
file_path = gen_filenames[file_conf.gen_filenames](file_conf.start_time, file_conf.end_time, file_conf.base_dir,
                                                   file_conf.filename_patterns['wind_speed'], file_conf.date_pattern,
...
fig_width = 7 * 0.393701  # cm 转换为英寸 图幅大小
fig_height = 4 * 0.393701  # cm 转换为英寸
figure_path = outputconfig['wind_speed']['preprocess']['raw_data_figure_path']
for i in range(len(file_path)):
    data, file_exists = Read_file.Read_csv1(file_path[i][1])
    channels=outputconfig['wind_speed']['preprocess']['channels']
    try :
    ...
    if file_exists :
        for j in range(len(figure_path)):
            ...
            channel=outputconfig['wind_speed']['preprocess']['channels'][j]
            ...
            plt.title(file_path[i][0].strftime("%y.%m.%d-%H")+f"时{channel}号风速仪时程图")
            plt.savefig(figure_path[j], format='png', dpi=300)  # 保存为 PNG 文件，300 dpi 清晰度
            plt.close()
        break
    if i==len(file_path)-1:
        print(f"原始风速数据缺失过多，无法绘制原始数据图，请放松阈值")
print('风速分析完成!')
