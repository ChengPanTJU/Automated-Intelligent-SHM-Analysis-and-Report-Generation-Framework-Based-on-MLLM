...
def VIC_rlt_read(folder_path, vic_conf):
    # 任务与分析函数的映射
    rlt_functions = {
        'preprocess': Prep_rlt,
        'rms': rms_rlt,  # rms函数
...
def oma_rlt(rlt, rlt_combined):
    rlt['OMA'] = rlt_combined['concat_ws(,, analysis_result[OMA] AS OMA)'].apply(convert_to_list)
    ...
    for channel in range(len(outputconfig['cable_vib']['OMA']['channels'])):
        ...
        plt.plot(x_t, fre1, label='频率',linewidth=0.6)
        # 设置 x 轴标签格式为 "日-月 时"
        ...
        fig1 = plt.figure(figsize=(fig_width, fig_height))
        dp1[dp1 < 0] = np.nan
        plt.plot(x_t, dp1*100, label='阻尼比',linewidth=0.4)
        # 设置 x 轴标签格式为 "日-月 时"
        plt.xlabel('时间 (月-日）', fontsize=12)
...
folder_path = r'D:\论文\04自动生成报告\监测数据自动化分析\result_folder_vic'
