...
from config import OutputConfig, StrConfig, FileConfig
import AnalysisFunction.analysis_methods as AM
OutConfig = OutputConfig()
outputconfig = OutConfig.tasks

def STR_rlt_read(folder_path, str_conf):
    # 任务与分析函数的映射
    rlt_functions = {
        'preprocess': Prep_rlt,
        'rms': rms_rlt,  # rms函数
...
def Prep_rlt(rlt, rlt_combined):
    ...
    preprocess_path = outputconfig['strain']['preprocess']['save_path']
    ...
    # 生成列表
    channels_list = outputconfig['strain']['preprocess']['channels']
    ytick_labels = [f"STR{str(i).zfill(2)}" for i in channels_list]
    ...
    plt.title(title_name[0] + "至" + title_name[len(x_t) - 1] + "应变传感器情况")
    # 调整图形布局
    plt.tight_layout()  # 自动调整布局以防止标签重叠
    figure_path = outputconfig['strain']['preprocess']['figure_path']
...
def rms_rlt(rlt, rlt_combined):
    ...
    rms_path = outputconfig['strain']['rms']['save_path']
    rms = pad_lists(rlt['rms'].tolist(), len(outputconfig['strain']['rms']['channels']))
    ...
    result = pd.DataFrame({
        '通道号': outputconfig['strain']['rms']['channels'],  # 设置第一列为通道号
        '应变均方根最大值(微应变)': save_data.max().values,  # 最大值
        '最大值时刻(月.日-时)': save_data.idxmax().values,  # 最大值所在的行号
        '应变均方根最小值(微应变)': save_data.min().values,  # 最小值
    ...
    sum_data_path = outputconfig['strain']['rms']['sum_table_path']
    result.to_csv(sum_data_path, index=False)
    ##############################绘图#################################
    pic_num = len(outputconfig['strain']['rms']['figure_path'])
    x_t = rlt['timestamp'].dt.tz_localize(None)  # 时间轴
    rms = save_data.values.tolist()
    rms = pad_lists(rms, len(outputconfig['strain']['rms']['channels']))
    for pic in range(pic_num):
        single_rms = [rms[i][pic] for i in range(len(rms))]
        figure_path = outputconfig['strain']['rms']['figure_path'][pic]
        channel_num = outputconfig['strain']['rms']['channels'][pic]
        fig = plt.figure(figsize=(fig_width, fig_height))
        plt.plot(x_t, single_rms, label='rms',linewidth=0.2)
        # 设置 x 轴标签格式为 "日-月 时"
        title_name = rlt['timestamp'].dt.strftime("%y.%m.%d")
        plt.title(f'{title_name[0]}至{title_name[len(x_t) - 1]}日{channel_num}号应变计均方根')
        plt.xlabel('时间 (月-日）')
        plt.ylabel('应变均方根 (微应变)')
...
def average_rlt(rlt, rlt_combined):
    ...
    mean_path = outputconfig['strain']['mean']['save_path']
    rms = pad_lists(rlt['mean'].tolist(), len(outputconfig['strain']['mean']['channels']))
    save_data = pd.DataFrame(rms, index=None)
    for i in range(save_data.shape[1]):
        original_data = save_data.iloc[:,i].copy()
        save_data.iloc[:,i],_=AM.rmoutliers_gesd(original_data)
    ...
    time_data = rlt['timestamp'].dt.strftime("%m.%d-%H")
    result = pd.DataFrame({
        '通道号': outputconfig['strain']['mean']['channels'],  # 设置第一列为通道号
        '应变均值最大值(微应变)': save_data.max().values,  # 最大值
        '最大值时刻(月.日-时)': save_data.idxmax().values,  # 最大值所在的行号
        '应变均值最小值(微应变)': save_data.min().values,  # 最小值
        '最小值时刻(月.日-时)': save_data.idxmin().values  # 最小值所在的行号
    })
    # 用 time_data 的索引值替换 result 中的索引
    result['最大值时刻(月.日-时)'] = result['最大值时刻(月.日-时)'].apply(
    ...
    # 填充 NaN
    result = result.fillna('通道数据缺失')
    sum_data_path = outputconfig['strain']['mean']['sum_table_path']
    result.to_csv(sum_data_path, index=False)
    ##############################绘图#################################
    pic_num = len(outputconfig['strain']['mean']['figure_path'])
    x_t = rlt['timestamp'].dt.tz_localize(None)  # 时间轴
    rms = save_data.values.tolist()
    rms = pad_lists(rms, len(outputconfig['strain']['mean']['channels']))
    for pic in range(pic_num):
        single_rms = [rms[i][pic] for i in range(len(rms))]
        figure_path = outputconfig['strain']['mean']['figure_path'][pic]
        channel_num = outputconfig['strain']['mean']['channels'][pic]
        fig = plt.figure(figsize=(fig_width, fig_height))
        plt.plot(x_t, single_rms, label='mean',linewidth=0.2)
        # 设置 x 轴标签格式为 "日-月 时"
        title_name = rlt['timestamp'].dt.strftime("%y.%m.%d")
        plt.title(f'{title_name[0]}至{title_name[len(x_t) - 1]}日{channel_num}号应变计均值')
        plt.xlabel('时间 (月-日）')
        plt.ylabel('应变均值(微应变)')
        plt.tick_params(axis='y')
        plt.tick_params(axis='x')
...
def max_min_rlt(rlt, rlt_combined):
    ...
    mean_path = outputconfig['strain']['max_min']['save_path']
    rms = pad_lists(rlt['max_min'].tolist(), len(outputconfig['strain']['max_min']['channels'])*2)
    save_data = pd.DataFrame(rms, index=None)
    for i in range(save_data.shape[1]):
        original_data = save_data.iloc[:,i].copy()
        save_data.iloc[:,i],_=AM.rmoutliers_gesd(original_data)
    ...
    result = pd.DataFrame({
        '通道号': outputconfig['strain']['max_min']['channels'],  # 设置第一列为通道号
        '应变最大值(微应变)': max_data.max().values,  # 最大值
        '最大值时刻(月.日-时)': max_data.idxmax().values,  # 最大值所在的行号
        '应变最小值(微应变)': min_data.min().values,  # 最小值
    ...
    # 填充 NaN
    result = result.fillna('通道数据缺失')
    sum_data_path = outputconfig['strain']['max_min']['sum_table_path']
    result.to_csv(sum_data_path, index=False)
    ##############################绘图#################################
    pic_num = len(outputconfig['strain']['max_min']['figure_path'])
    x_t = rlt['timestamp'].dt.tz_localize(None)  # 时间轴
    ...
    for pic in range(pic_num):
        ...
        figure_path = outputconfig['strain']['max_min']['figure_path'][pic]
        channel_num = outputconfig['strain']['max_min']['channels'][pic]
        fig = plt.figure(figsize=(fig_width, fig_height))
        plt.plot(x_t, single_max, label='max',linewidth=0.2)
        plt.plot(x_t, single_min, label='min',linewidth=0.2)
        # 设置 x 轴标签格式为 "日-月 时"
        title_name = rlt['timestamp'].dt.strftime("%y.%m.%d")
        plt.title(f'{title_name[0]}至{title_name[len(x_t) - 1]}日{channel_num}号应变计极值')
        plt.xlabel('时间 (月-日）')
        plt.ylabel('应变极值(微应变)')
        plt.tick_params(axis='y')
        plt.tick_params(axis='x')
...
folder_path = r'D:\论文\04自动生成报告\监测数据自动化分析\result_folder_str'
STR_rlt_read(folder_path, vicconfig)
...
file_path = gen_filenames[file_conf.gen_filenames](file_conf.start_time, file_conf.end_time, file_conf.base_dir,
                                                   file_conf.filename_patterns['strain'], file_conf.date_pattern,
...
fig_width = 7 * 0.393701  # cm 转换为英寸 图幅大小
fig_height = 4 * 0.393701  # cm 转换为英寸
figure_path = outputconfig['strain']['preprocess']['raw_data_figure_path']

for i in range(len(file_path)):
    data, file_exists = Read_file.Read_csv1(file_path[i][1])
    channels = outputconfig['strain']['preprocess']['channels']
    try:
    ...
    if file_exists:
        for j in range(len(figure_path)):
            ...
            channel=outputconfig['strain']['preprocess']['channels'][j]
            ...
            plt.title(file_path[i][0].strftime("%y.%m.%d-%H")+f"时{channel}号应变计时程图")
