...
from config import OutputConfig, WdrConfig, FileConfig
import AnalysisFunction.analysis_methods as AM
OutConfig = OutputConfig()
outputconfig = OutConfig.tasks

def WDR_rlt_read(folder_path, wdr_conf):
    # 任务与分析函数的映射
    rlt_functions = {
        'preprocess': Prep_rlt,
        'rms': rms_rlt,  # rms函数
...
vicconfig=WdrConfig()
folder_path = r'D:\论文\04自动生成报告\监测数据自动化分析\result_folder_wdr'
...
for i in range(len(file_path)):
    ...
    if file_exists:
        for j in range(len(figure_path)):
            ...
            channel=outputconfig['wind_direction']['preprocess']['channels'][j]
            ...
            plt.title(file_path[i][0].strftime("%y.%m.%d-%H")+f"时{channel}号风向仪时程图")
