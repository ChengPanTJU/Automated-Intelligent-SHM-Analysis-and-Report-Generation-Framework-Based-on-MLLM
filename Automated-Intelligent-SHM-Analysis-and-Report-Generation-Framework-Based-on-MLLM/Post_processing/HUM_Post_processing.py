...
from config import OutputConfig, HumConfig, FileConfig
import AnalysisFunction.analysis_methods as AM
OutConfig = OutputConfig()
outputconfig = OutConfig.tasks

def HUM_rlt_read(folder_path, hum_conf):
    # 任务与分析函数的映射
    rlt_functions = {
        'preprocess': Prep_rlt,
        'rms': rms_rlt,  # rms函数
    ...
    # 分析对应任务的结果
    for task, channels in hum_conf.tasks_channels.items():
...
def Prep_rlt(rlt, rlt_combined):
    ...
    preprocess_path = outputconfig['humidity']['preprocess']['save_path']
    ...
    # 生成列表
    channels_list = outputconfig['humidity']['preprocess']['channels']
    ytick_labels = [f"HUM{str(i).zfill(2)}" for i in channels_list]
    ...
    plt.title(title_name[0] + "至" + title_name[len(x_t) - 1] + "湿度传感器情况")
    # 调整图形布局
    plt.tight_layout()  # 自动调整布局以防止标签重叠
    figure_path = outputconfig['humidity']['preprocess']['figure_path']
...
def rms_rlt(rlt, rlt_combined):
    ...
    rms_path = outputconfig['humidity']['rms']['save_path']
    rms = pad_lists(rlt['rms'].tolist(), len(outputconfig['humidity']['rms']['channels']))
    ...
    result = pd.DataFrame({
        '通道号': outputconfig['humidity']['rms']['channels'],  # 设置第一列为通道号
        '湿度均方根最大值(%)': save_data.max().values,  # 最大值
        '最大值时刻(月.日-时)': save_data.idxmax().values,  # 最大值所在的行号
        '湿度均方根最小值(%)': save_data.min().values,  # 最小值
    ...
    sum_data_path = outputconfig['humidity']['rms']['sum_table_path']
    result.to_csv(sum_data_path, index=False)
    ##############################绘图#################################
    pic_num = len(outputconfig['humidity']['rms']['figure_path'])
    x_t = rlt['timestamp'].dt.tz_localize(None)  # 时间轴
    rms = save_data.values.tolist()
    rms = pad_lists(rms, len(outputconfig['humidity']['rms']['channels']))
    for pic in range(pic_num):
        single_rms = [rms[i][pic] for i in range(len(rms))]
        figure_path = outputconfig['humidity']['rms']['figure_path'][pic]
        channel_num = outputconfig['humidity']['rms']['channels'][pic]
        fig = plt.figure(figsize=(fig_width, fig_height))
        plt.plot(x_t, single_rms, label='rms',linewidth=0.2)
        # 设置 x 轴标签格式为 "日-月 时"
        title_name = rlt['timestamp'].dt.strftime("%y.%m.%d")
        plt.title(f'{title_name[0]}至{title_name[len(x_t) - 1]}日{channel_num}号湿度计均方根')
        plt.xlabel('时间 (月-日）')
        plt.ylabel('相对湿度均方根 (%)')
...
def average_rlt(rlt, rlt_combined):
    ...
    mean_path = outputconfig['humidity']['mean']['save_path']
    rms = pad_lists(rlt['mean'].tolist(), len(outputconfig['humidity']['mean']['channels']))
    save_data = pd.DataFrame(rms, index=None)
    for i in range(save_data.shape[1]):
        original_data = save_data.iloc[:,i].copy()
        save_data.iloc[:,i],_=AM.rmoutliers_gesd(original_data)
    ...
    time_data = rlt['timestamp'].dt.strftime("%m.%d-%H")
    result = pd.DataFrame({
        '通道号': outputconfig['humidity']['mean']['channels'],  # 设置第一列为通道号
        '湿度均值最大值(%)': save_data.max().values,  # 最大值
        '最大值时刻(月.日-时)': save_data.idxmax().values,  # 最大值所在的行号
        '湿度均值最小值(%)': save_data.min().values,  # 最小值
        '最小值时刻(月.日-时)': save_data.idxmin().values  # 最小值所在的行号
    })
    # 用 time_data 的索引值替换 result 中的索引
    result['最大值时刻(月.日-时)'] = result['最大值时刻(月.日-时)'].apply(
    ...
    # 填充 NaN
    result = result.fillna('通道数据缺失')
    sum_data_path = outputconfig['humidity']['mean']['sum_table_path']
    result.to_csv(sum_data_path, index=False)
    ##############################绘图#################################
    pic_num = len(outputconfig['humidity']['mean']['figure_path'])
    x_t = rlt['timestamp'].dt.tz_localize(None)  # 时间轴
    rms = save_data.values.tolist()
    rms=pad_lists(rms,len(outputconfig['humidity']['mean']['channels']))
    for pic in range(pic_num):
        single_rms = [rms[i][pic] for i in range(len(rms))]
        figure_path = outputconfig['humidity']['mean']['figure_path'][pic]
        channel_num = outputconfig['humidity']['mean']['channels'][pic]
        fig = plt.figure(figsize=(fig_width, fig_height))
        plt.plot(x_t, single_rms, label='mean',linewidth=0.2)
        # 设置 x 轴标签格式为 "日-月 时"
        title_name = rlt['timestamp'].dt.strftime("%y.%m.%d")
        plt.title(f'{title_name[0]}至{title_name[len(x_t) - 1]}日{channel_num}号湿度计均值')
        plt.xlabel('时间 (月-日）')
        plt.ylabel('相对湿度均值(%)')
        plt.tick_params(axis='y')
        plt.tick_params(axis='x')
...
from config import HumConfig
vicconfig=HumConfig()

folder_path = r'D:\论文\04自动生成报告\监测数据自动化分析\result_folder_hum'
HUM_rlt_read(folder_path, vicconfig)
...
file_path = gen_filenames[file_conf.gen_filenames](file_conf.start_time, file_conf.end_time, file_conf.base_dir,
                                                   file_conf.filename_patterns['humidity'], file_conf.date_pattern,
...
fig_width = 7 * 0.393701  # cm 转换为英寸 图幅大小
fig_height = 4 * 0.393701  # cm 转换为英寸
figure_path = outputconfig['humidity']['preprocess']['raw_data_figure_path']
for i in range(len(file_path)):
    data, file_exists = Read_file.Read_csv1(file_path[i][1])
    channels = outputconfig['humidity']['preprocess']['channels']
    try:
    ...
    if file_exists:
        for j in range(len(figure_path)):
            ...
            channel=outputconfig['humidity']['preprocess']['channels'][j]
            ...
            plt.title(file_path[i][0].strftime("%y.%m.%d-%H")+f"时{channel}号湿度计时程图")
