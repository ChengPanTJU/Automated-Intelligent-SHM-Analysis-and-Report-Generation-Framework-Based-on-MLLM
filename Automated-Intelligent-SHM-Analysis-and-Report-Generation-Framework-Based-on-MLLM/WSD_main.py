...
from AnalysisFunction import Wsd_Analyze
from DataRead import Read_file
from Post_processing import WSD_Post_processing
from config import FileConfig, WsdConfig, OutputConfig
...
# 将自定义分析函数注册为 UDF
analyze_file_udf = F.udf(lambda File_path:Wsd_Analyze.Wsd_analyze(File_path, wsd_conf),
...
for task in wsd_conf.tasks_channels:
...
df_with_selected_columns.coalesce(10).write.csv("result_folder_wsd", header=True, mode='overwrite')
...
folder_path = os.path.join(current_dir, 'result_folder_wsd')
os.makedirs(folder_path, exist_ok=True)
# 将结果分别保存到csv文件中用于后续绘图分析
WSD_Post_processing.WSD_rlt_read(folder_path, wsd_conf)
# 关闭当前会话
spark.stop()
