...
def Wsd_analyze(df_wsd, wsd_conf):
    """根据指定的任务执行相应的分析"""
    # 读取文件内容
    file_path = df_wsd['file_path']
...
def Prep_WSD(data, wsd_conf):
    ...
    np_array = np.array([float(x) for x in result])
    result = np_array.tolist()
    return result

def average(data, para_conf=0):
    rows = data.shape[0]
    split_data = [data.iloc[i * rows // 6 : (i + 1) * rows // 6, :] for i in range(6)]
    avg_values = [part.mean(axis=0).values.tolist() for part in split_data]
    return max(avg_values)

def rms(data, para_conf=0):
    """计算数据的均方根（RMS）"""
    return data.std(axis=0).values.tolist()
