...
import numpy as np
import pandas as pd
from config import FileConfig, IanConfig
from DataRead import Read_file

def Ian_analyze(df_isn, ian_conf):
    """根据指定的任务执行相应的分析"""
    # 读取文件内容
    file_path = df_isn['file_path']
    ...
    for task, channels in ian_conf.tasks_channels.items():
        if task in analysis_functions.keys():
            ...
            if any(channel >= data.shape[1] or channel < 0 for channel in channels):
                ...
                results[task] = analysis_func(data.iloc[:, channels], ian_conf)
        else:
...
def Prep_Ian(data, ian_conf):
    ...
    for i in range(num_columns):
        ...
        ############判断离群点（阈值判断）#############
        if max(new_data) > ian_conf.Pre_conf["up_lim"] or min(new_data) < ian_conf.Pre_conf["low_lim"]:
            outlier_index = 1
        else:
            outlier_index = 0
...
if __name__ == '__main__':
    ...
    rlt = Ian_analyze(datapath, tmp_conf)
