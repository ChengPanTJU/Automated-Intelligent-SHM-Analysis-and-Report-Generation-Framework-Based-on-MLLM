...
from config import DisConfig, OutputConfig
OutConfig = OutputConfig()
outputconfig = OutConfig.tasks

def Dis_analyze(df_dis, dis_conf):
    """根据指定的任务执行相应的分析"""
    file_path = df_dis['file_path']
    # 读取文件内容
    ...
    for task, channels in dis_conf.tasks_channels.items():
        if task in analysis_functions.keys():
            ...
            if any(channel >= data.shape[1] or channel < 0 for channel in channels):
                ...
                results[task] = analysis_func(data.iloc[:, channels], dis_conf)
        else:
...
def average(data, para_conf=0):
    ...
    cbfconf = DisConfig()
    # 先创建 DataFrame 副本，避免修改原数据导致 SettingWithCopyWarning
    data = data.copy()
    if cbfconf.fs <= 1:
    ...
    for col in data.columns:
        unique_values = data[col].dropna().unique()  # 去除 NaN 后的唯一值
        if len(unique_values) <= 5:  # 只有一个唯一值或全是 NaN
            data.loc[:, col] = np.nan
...
def rms(data, para_conf=0):
    ...
    for col in data.columns:
        unique_values = data[col].dropna().unique()  # 去除 NaN 后的唯一值
        if len(unique_values) <= 5:  # 只有一个唯一值或全是 NaN
            data.loc[:, col] = np.nan  # 这里用 .loc 赋值，避免 SettingWithCopyWarning
...
if __name__ == '__main__':
    ...
    datapath = r'G:\宁波数据\湾头大桥\data\2020-11-01\2020-11-01 00-DPM.csv'
    # 自动识别方法
    rlt = Dis_analyze(datapath, acc_conf)
    1
