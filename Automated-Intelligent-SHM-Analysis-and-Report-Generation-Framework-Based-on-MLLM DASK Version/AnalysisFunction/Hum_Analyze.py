...
import numpy as np
import pandas as pd
from config import FileConfig, HumConfig
from DataRead import Read_file


def Hum_analyze(df_hum, hum_conf):
    """根据指定的任务执行相应的分析"""
    file_path = df_hum['file_path']
    # 读取文件内容
    ...
    for task, channels in hum_conf.tasks_channels.items():
        if task in analysis_functions.keys():
            ...
            if any(channel >= data.shape[1] or channel < 0 for channel in channels):
                ...
                results[task] = analysis_func(data.iloc[:, channels], hum_conf)
        else:
...
def Prep_Hum(data, hum_conf):
    ...
    for i in range(num_columns):
        ...
        ############判断离群点（阈值判断）#############
        if max(new_data) > hum_conf.Pre_conf["up_lim"] or min(new_data) < hum_conf.Pre_conf["low_lim"]:
            outlier_index = 1
        else:
            outlier_index = 0
...
if __name__ == '__main__':
    ...
    datapath = 'F:/博士资料/监测数据自动化分析-12.8/test_data/2020-01-27 00-TMP.csv'
    data, _ = Read_file.Read_csv1(datapath)
    rlt = Hum_analyze(datapath, hum_conf)
