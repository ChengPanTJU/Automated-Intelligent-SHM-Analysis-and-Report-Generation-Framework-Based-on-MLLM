...
from config import SetConfig, OutputConfig
OutConfig = OutputConfig()
outputconfig = OutConfig.tasks

def Set_analyze(df_set, set_conf):
    """根据指定的任务执行相应的分析"""
    # 读取文件内容
    file_path = df_set['file_path']
    ...
    for task, channels in set_conf.tasks_channels.items():
        if task in analysis_functions.keys():
            ...
            if any(channel >= data.shape[1] or channel < 0 for channel in channels):
                ...
                results[task] = analysis_func(data.iloc[:, channels], set_conf)
        else:
