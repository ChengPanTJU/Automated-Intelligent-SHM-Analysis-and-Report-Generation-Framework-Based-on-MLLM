...
from config import GpsConfig, OutputConfig
OutConfig = OutputConfig()
outputconfig = OutConfig.tasks

def Gps_analyze(df_gps, gps_conf):
    """根据指定的任务执行相应的分析"""
    file_path = df_gps['file_path']
    # 读取文件内容
    ...
    for task, channels in gps_conf.tasks_channels.items():
        if task in analysis_functions.keys():
            ...
            if any(channel >= data.shape[1] or channel < 0 for channel in channels):
                ...
                results[task] = analysis_func(data.iloc[:, channels], gps_conf)
        else:
