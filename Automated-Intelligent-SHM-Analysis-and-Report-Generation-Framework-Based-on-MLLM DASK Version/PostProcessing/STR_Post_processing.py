...
from config import OutputConfig, StrConfig
import AnalysisFunction.analysis_methods as AM
OutConfig = OutputConfig()
outputconfig = OutConfig.tasks
current_dir = os.path.dirname(os.path.abspath(__file__))
results_dir = os.path.join(current_dir,'strain','rlt_table')
# 确保目标文件夹存在
os.makedirs(results_dir, exist_ok=True)
results_dir = os.path.join(current_dir,'strain','rlt_figure')
os.makedirs(results_dir, exist_ok=True)
def STR_rlt_read(file_path, str_conf):
    # 任务与分析函数的映射
    rlt_functions = {
        'preprocess': Prep_rlt,
        'rms': rms_rlt,  # rms函数
...
if __name__ == '__main__':
    ...
    folder_path = r'G:\博士资料\监测数据自动化分析-dask\RowResults\STR_rlt_row.csv'
