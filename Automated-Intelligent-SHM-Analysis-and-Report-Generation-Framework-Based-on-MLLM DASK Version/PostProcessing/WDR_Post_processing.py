...
from config import OutputConfig, WdrConfig
import AnalysisFunction.analysis_methods as AM
OutConfig = OutputConfig()
outputconfig = OutConfig.tasks
current_dir = os.path.dirname(os.path.abspath(__file__))
results_dir = os.path.join(current_dir,'wind_direction','rlt_table')
# 确保目标文件夹存在
os.makedirs(results_dir, exist_ok=True)
results_dir = os.path.join(current_dir,'wind_direction','rlt_figure')
os.makedirs(results_dir, exist_ok=True)
# 确保目标文件夹存在
os.makedirs(results_dir, exist_ok=True)
def WDR_rlt_read(file_path, wdr_conf):
    # 任务与分析函数的映射
    rlt_functions = {
        'preprocess': Prep_rlt,
        'rms': rms_rlt,  # rms函数
...
def Prep_rlt(rlt):
    ...
    preprocess_path = outputconfig['wind_direction']['preprocess']['save_path']
    ...
    # 生成列表
    channels_list = outputconfig['wind_direction']['preprocess']['channels']
    ytick_labels = [f"WDR{str(i).zfill(2)}" for i in channels_list]
    ...
    plt.title(title_name[0] + "至" + title_name[len(x_t) - 1] + "风向传感器情况")
    # 调整图形布局
    plt.tight_layout()  # 自动调整布局以防止标签重叠
    figure_path = outputconfig['wind_direction']['preprocess']['figure_path']
...
def rms_rlt(rlt):
    ...
    rms_path = outputconfig['wind_direction']['rms']['save_path']
    rms = pad_lists(rlt['rms'].tolist(), len(outputconfig['wind_direction']['rms']['channels']))
    ...
    pic_num = len(outputconfig['wind_direction']['rms']['figure_path'])
    x_t = rlt['timestamp'].dt.tz_localize(None)  # 时间轴
    rms = save_data.values.tolist()
    rms = pad_lists(rms, len(outputconfig['wind_direction']['rms']['channels']))
    for pic in range(pic_num):
        single_rms = [rms[i][pic] for i in range(len(rms))]
        figure_path = outputconfig['wind_direction']['rms']['figure_path'][pic]
        channel_num = outputconfig['wind_direction']['rms']['channels'][pic]
        fig = plt.figure(figsize=(fig_width, fig_height))
        plt.plot(x_t, single_rms, label='rms',linewidth=0.2)
        # 设置 x 轴标签格式为 "日-月 时"
        title_name = rlt['timestamp'].dt.strftime("%y.%m.%d")
        plt.title(f'{title_name[0]}至{title_name[len(x_t) - 1]}日{channel_num}号风向仪均方根')
        plt.xlabel('时间 (月-日）')
        plt.ylabel('风向均方根 (°)')
...
def average_rlt(rlt):
    ...
    mean_path = outputconfig['wind_direction']['mean']['save_path']
    rms = pad_lists(rlt['mean'].tolist(), len(outputconfig['wind_direction']['mean']['channels']))
    save_data = pd.DataFrame(rms, index=None)
    for i in range(save_data.shape[1]):
        original_data = save_data.iloc[:,i].copy()
        save_data.iloc[:,i],_=AM.rmoutliers_gesd(original_data)
    np.savetxt(mean_path, save_data, delimiter=',')
    pic_num = len(outputconfig['wind_direction']['mean']['channels'])
    x_t = rlt['timestamp'].dt.tz_localize(None)  # 时间轴
    rms = save_data.values.tolist()
    rms = pad_lists(rms, len(outputconfig['wind_direction']['mean']['channels']))
    for pic in range(pic_num):
        single_rms = [rms[i][pic] for i in range(len(rms))]
        figure_path = outputconfig['wind_direction']['mean']['figure_path'][2*pic]
        channel_num = outputconfig['wind_direction']['mean']['channels'][pic]
        fig = plt.figure(figsize=(fig_width, fig_height))
        plt.plot(x_t, single_rms, label='mean',linewidth=0.2)
        # 设置 x 轴标签格式为 "日-月 时"
        title_name = rlt['timestamp'].dt.strftime("%y.%m.%d")
        plt.title(f'{title_name[0]}至{title_name[len(x_t) - 1]}日{channel_num}号风向仪均值')
        plt.xlabel('时间 (月-日）')
        plt.ylabel('风向均值(°)')
        plt.tick_params(axis='y')
        plt.tick_params(axis='x')
        ...
        figure_path = outputconfig['wind_direction']['mean']['figure_path'][2*pic+1]
        channel_num = outputconfig['wind_direction']['mean']['channels'][pic]
        # 定义风向区间（8个方向，每个方向45°）
        direction_labels = ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW']
        direction_bins = np.arange(-22.5, 361, 45)  # 创建8个方向的边界
        # 计算每个方向的风向频率
        wind_counts, _ = np.histogram(single_rms, bins=direction_bins)
        wind_frequencies = wind_counts / wind_counts.sum()  # 归一化为频率
        # 转换角度为弧度
        angles = np.deg2rad(np.linspace(0, 360, num=9))[:-1]  # 0到360度，共8个方向
        angles = np.append(angles, angles[0])  # 闭合图形
        # 频率数据闭合
        wind_frequencies = np.append(wind_frequencies, wind_frequencies[0])
        # 绘制极坐标玫瑰图
        fig, ax = plt.subplots(subplot_kw={'projection': 'polar'},figsize=(fig_width, fig_height))
        ax.set_theta_zero_location('N')  # 以北方为起点
        ax.set_theta_direction(-1)  # 顺时针方向
        # 绘制填充玫瑰图
        ax.fill(angles, wind_frequencies, color='blue', alpha=0.5)
        ax.plot(angles, wind_frequencies, color='blue', linewidth=2)
        # 设置角度刻度
        ax.set_xticks(np.deg2rad(np.arange(0, 360, 45)))
        ax.set_xticklabels(direction_labels)
        ax.set_title(f"{title_name[0]}至{title_name[len(x_t) - 1]}风向仪{channel_num}号通道风向频率玫瑰图")
        plt.tight_layout()  # 自动调整布局以防止标签重叠
        plt.savefig(figure_path, format='png', dpi=300)  # 保存为 PNG 文件，300 dpi 清晰度
        plt.close(fig)  # 关闭当前图表以避免影响后面的图表
...
if __name__ == '__main__':
    from config import WdrConfig
    vicconfig=WdrConfig()
    folder_path = r"D:\自动化程序\监测数据自动化分析-dask\RowResults\WDR_rlt_row.csv"
    WDR_rlt_read(folder_path, vicconfig)
