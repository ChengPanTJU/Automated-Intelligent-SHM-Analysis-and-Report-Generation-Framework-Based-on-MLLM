...
from config import  OutputConfig,WsdConfig
import AnalysisFunction.analysis_methods as AM
OutConfig = OutputConfig()
outputconfig = OutConfig.tasks
current_dir = os.path.dirname(os.path.abspath(__file__))
results_dir = os.path.join(current_dir,'wind_speed','rlt_table')
# 确保目标文件夹存在
os.makedirs(results_dir, exist_ok=True)
results_dir = os.path.join(current_dir,'wind_speed','rlt_figure')
# 确保目标文件夹存在
os.makedirs(results_dir, exist_ok=True)

def WSD_rlt_read(file_path, wsd_conf):
    # 任务与分析函数的映射
    rlt_functions = {
        'preprocess': Prep_rlt,
        'rms': rms_rlt,  # rms函数
...
if __name__ == '__main__':
    ...
    folder_path = r'D:\自动化程序\监测数据自动化分析\result_folder_wsd'
