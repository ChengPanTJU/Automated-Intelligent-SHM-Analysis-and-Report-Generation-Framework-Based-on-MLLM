...
results_dir = os.path.join(current_dir,'vibration','rlt_table')
# 确保目标文件夹存在
os.makedirs(results_dir, exist_ok=True)
results_dir = os.path.join(current_dir,'vibration','rlt_figure')
os.makedirs(results_dir, exist_ok=True)
import AnalysisFunction.analysis_methods as AM
def ACC_rlt_read(file_path, acc_conf):
    # 任务与分析函数的映射
    rlt_functions = {
        'preprocess': Prep_rlt,
        'rms': rms_rlt,  # rms函数
...
def oma_rlt(rlt):
    rlt['OMA1'] = rlt['OMA1'].apply(convert_to_list)
...
def oma_rlt2(rlt):
    rlt['OMA2'] = rlt['OMA2'].apply(convert_to_list)
...
def oma_rlt3(rlt):
    rlt['OMA3'] = rlt['OMA3'].apply(convert_to_list)
...
if __name__ == '__main__':
    ...
    file_path = r'D:\自动化程序\监测数据自动化分析-dask\RowResults\ACC_rlt_row.csv'

    acc_conf = AccConfig()
    ACC_rlt_read(file_path, acc_conf)
