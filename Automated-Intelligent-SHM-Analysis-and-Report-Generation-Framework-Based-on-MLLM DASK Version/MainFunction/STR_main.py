...
from AnalysisFunction import Str_Analyze
from PostProcessing import STR_Post_processing
from config import FileConfig, StrConfig, OutputConfig
...
def process_partition(df):
    ...
    # 将字典重新包装成 StrConfig 实例（如果需要）
    str_conf_obj = StrConfig()
    result_df = df.apply(lambda row: Str_Analyze.Str_analyze(row, str_conf_obj), axis=1)
    # 保留原来的行索引
...
def plot_raw_timeseries(
    ...
    for ch_idx, channel in enumerate(channels):
        ...
        for time_tag, csv_path in file_path_list:
            ...
            ax.set_ylabel('应变（微应变）')
            ax.set_title(
                time_tag.strftime("%y.%m.%d-%H") + f" 时{channel}号应变计时程图"
            )
        ...
        if not plotted:
            print(f"⚠️ 应变 {channel} 号通道在所有时段均无有效数据，未绘制原始时程图")
def STR_main(num_workers=4):
    # --- 配置多核并行 ---
    # 获取当前脚本所在路径
    ...
    rlt_path = os.path.join(folder_path, 'STR_rlt_row.csv')
    ...
    # 转为Dask的df数组
    ddf_dask = dd.from_pandas(df_pd, npartitions=num_workers * 2)

    # 对每个分区应用Acc_Analyze
    meta = {task: object for task in str_conf.tasks_channels.keys()}
    ...
    STR_Post_processing.STR_rlt_read(rlt_path, str_conf)
    ################## 绘制原始数据的时程图 ##################
