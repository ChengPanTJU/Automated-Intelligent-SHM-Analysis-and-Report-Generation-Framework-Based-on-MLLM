...
from PostProcessing import SET_Post_processing
...
def process_partition(df):
    ...
    set_conf_obj = SetConfig()
    # acc_conf_obj.__dict__.update(acc_conf_dict)
    result_df = df.apply(lambda row: Set_Analyze.Set_analyze(row, set_conf_obj), axis=1)
    # 保留原来的行索引
...
def plot_raw_timeseries(
    ...
    for ch_idx, channel in enumerate(channels):
        ...
        if not plotted:
            print(f"⚠️ 结构变形 {channel} 号通道在所有时段均无有效数据，未绘制原始时程图")
def SET_main(num_workers=4):
    # --- 配置多核并行 ---
    # num_workers = 4
    # 获取当前脚本所在路径
    ...
    rlt_path = os.path.join(folder_path, 'SET_rlt_row.csv')
    ...
    # 对每个分区应用Acc_Analyze
    meta = {task: object for task in set_conf.tasks_channels.keys()}
    ...
    # 结果后处理
    SET_Post_processing.SET_rlt_read(rlt_path, set_conf)
