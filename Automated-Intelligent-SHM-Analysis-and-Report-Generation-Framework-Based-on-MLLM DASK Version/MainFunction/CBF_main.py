...
from PostProcessing import CBF_Post_processing
...
def process_partition(df):
    ...
    cbf_conf_obj = CbfConfig()
    result_df = df.apply(lambda row: Cbf_Analyze.Cbf_analyze(row, cbf_conf_obj), axis=1)
    # 保留原来的行索引
...
def plot_raw_timeseries(
    ...
    for ch_idx, channel in enumerate(channels):
        ...
        if not plotted:
            print(f"⚠️ 索力 {channel} 号通道在所有时段均无有效数据，未绘制原始时程图")
def CBF_main(num_workers=4):
    # --- 配置多核并行 ---
    # num_workers = 4
    # 获取当前脚本所在路径
    ...
    rlt_path = os.path.join(folder_path, 'CBF_rlt_row.csv')
    ...
    # 对每个分区应用Acc_Analyze
    meta = {task: object for task in cbf_conf.tasks_channels.keys()}
    ...
    # 结果后处理
    CBF_Post_processing.CBF_rlt_read(rlt_path, cbf_conf)
    ################## 绘制原始数据的时程图 ##################
