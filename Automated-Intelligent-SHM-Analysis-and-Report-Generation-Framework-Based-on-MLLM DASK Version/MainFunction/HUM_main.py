...
from PostProcessing import HUM_Post_processing
...
def process_partition(df):
    ...
    hum_conf_obj = HumConfig()
    result_df = df.apply(lambda row: Hum_Analyze.Hum_analyze(row, hum_conf_obj), axis=1)
    # 保留原来的行索引
...
def plot_raw_timeseries(
    ...
    for ch_idx, channel in enumerate(channels):
        ...
        for time_tag, csv_path in file_path_list:
            ...
            ax.set_ylabel('湿度（%）')
            ax.set_title(
                time_tag.strftime("%y.%m.%d-%H") + f" 时{channel}号湿度计时程图"
            )
        ...
        if not plotted:
            print(f"⚠️ 湿度 {channel} 号通道在所有时段均无有效数据，未绘制原始时程图")
def HUM_main(num_workers=4):
    # --- 配置多核并行 ---
    # num_workers = 4
    # 获取当前脚本所在路径
    ...
    rlt_path = os.path.join(folder_path, 'HUM_rlt_row.csv')
    ...
    # 对每个分区应用Acc_Analyze
    meta = {task: object for task in hum_conf.tasks_channels.keys()}
    ...
    # 结果后处理
    HUM_Post_processing.HUM_rlt_read(rlt_path, hum_conf)
    ################## 绘制原始数据的时程图 ##################
