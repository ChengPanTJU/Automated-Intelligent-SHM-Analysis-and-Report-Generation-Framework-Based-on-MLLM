...
from PostProcessing import VIC_Post_processing
...
def process_partition(df):
    ...
    vic_conf_obj = VicConfig()
    # acc_conf_obj.__dict__.update(acc_conf_dict)
    result_df = df.apply(lambda row: Vic_Analyze.Vic_analyze(row, vic_conf_obj), axis=1)
    # 保留原来的行索引
...
def plot_raw_timeseries(
    ...
    for ch_idx, channel in enumerate(channels):
        ...
        for time_tag, csv_path in file_path_list:
            ...
            ax.set_ylabel('拉索加速度（mg）')
            ax.set_title(
                time_tag.strftime("%y.%m.%d-%H") + f" 时{channel}号拉索加速度计时程图"
            )
        ...
        if not plotted:
            print(f"⚠️ 索力加速度 {channel} 号通道在所有时段均无有效数据，未绘制原始时程图")
def VIC_main(num_workers=4):
    # --- 配置多核并行 ---
    # num_workers = 4
    # 获取当前脚本所在路径
    ...
    rlt_path = os.path.join(folder_path, 'VIC_rlt_row.csv')
    ...
    # 对每个分区应用Acc_Analyze
    meta = {task: object for task in vic_conf.tasks_channels.keys()}
    ...
    # 结果后处理
    VIC_Post_processing.VIC_rlt_read(rlt_path, vic_conf)
    ################## 绘制原始数据的时程图 ##################
