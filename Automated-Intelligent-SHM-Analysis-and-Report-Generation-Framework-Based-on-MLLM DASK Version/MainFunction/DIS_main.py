...
from PostProcessing import DIS_Post_processing
...
def process_partition(df):
    ...
    str_conf_obj = DisConfig()
    result_df = df.apply(lambda row: Dis_Analyze.Dis_analyze(row, str_conf_obj), axis=1)
    # 保留原来的行索引
...
def plot_raw_timeseries(
    ...
    for ch_idx, channel in enumerate(channels):
        ...
        for time_tag, csv_path in file_path_list:
            ...
            ax.set_ylabel('梁端位移（mm）')
            ax.set_title(
                time_tag.strftime("%y.%m.%d-%H") + f" 时{channel}号梁端位移计时程图"
            )
        ...
        if not plotted:
            print(f"⚠️ 梁端位移 {channel} 号通道在所有时段均无有效数据，未绘制原始时程图")

def DIS_main(num_workers=4):
    # --- 配置多核并行 ---
    # num_workers = 4
    # 获取当前脚本所在路径
    ...
    rlt_path = os.path.join(folder_path, 'DIS_rlt_row.csv')
    ...
    # 对每个分区应用Dis_Analyze
    meta = {task: object for task in dis_conf.tasks_channels.keys()}
    ...
    # 后处理
    DIS_Post_processing.DIS_rlt_read(rlt_path, dis_conf)
    ################## 绘制原始数据的时程图 ##################
