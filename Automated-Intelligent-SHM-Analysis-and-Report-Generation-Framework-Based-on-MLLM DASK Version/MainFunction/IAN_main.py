...
from PostProcessing import IAN_Post_processing
...
def process_partition(df):
    ...
    ian_conf_obj = IanConfig()
    # acc_conf_obj.__dict__.update(acc_conf_dict)
    result_df = df.apply(lambda row: Ian_Analyze.Ian_analyze(row, ian_conf_obj), axis=1)
    # 保留原来的行索引
...
def plot_raw_timeseries(
    ...
    for ch_idx, channel in enumerate(channels):
        ...
        for time_tag, csv_path in file_path_list:
            ...
            ax.set_ylabel('倾角（°）')
            ax.set_title(
                time_tag.strftime("%y.%m.%d-%H") + f" 时{channel}号倾角仪时程图"
            )
        ...
        if not plotted:
            print(f"⚠️ 倾角 {channel} 号通道在所有时段均无有效数据，未绘制原始时程图")
def IAN_main(num_workers=4):
    # --- 配置多核并行 ---
    # num_workers = 4
    # 获取当前脚本所在路径
    ...
    rlt_path = os.path.join(folder_path, 'IAN_rlt_row.csv')
    ...
    # 对每个分区应用Acc_Analyze
    meta = {task: object for task in ian_conf.tasks_channels.keys()}
    ...
    # 结果后处理
    IAN_Post_processing.IAN_rlt_read(rlt_path, ian_conf)
    ################## 绘制原始数据的时程图 ##################
